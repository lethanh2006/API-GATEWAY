...
npm run lint
CI=true NODE_ENV=test OBSERVABILITY_LOAD_DOTENV=false OTEL_SDK_DISABLED=true LOG_FORMAT=json npm test
```
...
build image `linux/amd64` của service `gateway`, rồi truyền image qua SSH
...
`~/.ssh/nrapp_github_cd_gateway`; không dùng key SSH cá nhân hoặc key của
